      ...
      <form @submit.prevent="handleSubmit">
        ...
        <div class="form-group" v-if="task.is_recurring && task.recurrence_rule === 'weekly'">
          ...
          <div class="radio-group days">
            <label v-for="day in daysOfWeek" :key="day.value" class="day-pill">
...
const rules = computed(() => ({
  task: {
    title: { required },
    start_date: { notPastDate },
    end_date: { notPastDate, endDateAfterStartDate }
  }
}))
...
const handleSubmit = async () => {
  ...
  try {
    ...
    console.error('Task could not be updated:', error.response?.data || error.message)
  }
}
</script>
